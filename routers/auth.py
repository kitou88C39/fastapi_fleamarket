...
DbDependency = Annotated[OAuth2PasswordRequestForm, Depends()]
...
async def login(db: DbDependency, from_data: FormDependency):
    user = auth_cruds.authenticate_user(db, from_data.username, from_data.password)
    if not user:
